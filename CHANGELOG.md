...
Consumer migration (marketing site): `<Footer hiddenLinks={[...]} />` is consumed in
`Risqbase-Inc/RisqBase`. The local `src/components/layout/MarketingFooter.tsx` is
retained as fallback until the upstream component is verified stable in production.
