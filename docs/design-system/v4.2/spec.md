...
**Contributors:** Priya Sharma (Frontend + UI), Alex Chen (Technical Lead), Sophie Brennan (Growth & Customer Success)
**Approved by:** Fiyin Adeleke (CEO)
...
> **System scope.** The RisqBase Design System is the parent design baseline for **all** RisqBase products: RALIA (ralia.io), RisqBase marketing (risqbase.com), and Cortex (cortex.risqbase.com). Product-specific signature components remain product-specific only when they cannot be expressed without product-specific semantics; the *system* is RisqBase-wide.
...
| Letter | RALIA (ralia.io) | Cortex (cortex.risqbase.com) | RisqBase marketing |
|--------|------------------|------------------------------|---------------------|
...
Approval by Elena. Recorded in this section. Cannot duplicate or alias an existing noun.
...
Any future gauge is a configured instance, not a new primitive. Adding a new visual gauge variant requires Elena's approval and updates §7.11.
...
This test is mandatory in pattern recipes that contain `composed_of: [chart, ...]`. Reviewers (Elena) reject pattern PRs that skip it.
...
This list is normative. Adding to it requires Elena's approval. **Removing** from it requires unanimous approval from Elena, Priya, and Alex.
...
- The disputed-borders rendering needs review by Sophie's team for any customer-facing political sensitivity in the Horizon roll-out
...
Tone matrices for new surfaces: choose the closest row above. New rows added by Sophie's approval.
...
Templates:
  ...
  - Conflict error: "{Specific conflict}. {Specific resolution}." → "This assessment was edited by Priya 30 seconds ago. Refresh to see the latest version."
...
A new field requires inline help at minimum. A new surface requires inline help **and** a Knowledge Bank article. Sophie's team owns the Knowledge Bank.
...
| Quarterly **adoption report** to Elena, Priya, Alex, Sophie | Quarterly |
...
This is non-trivial. We are a compliance product. A telemetry programme that breaches our own customer DPA terms is a category of incident we do not survive. Sophie's team owns disclosure language. Alex's team owns the technical implementation. Both must sign off before v4.2.0 ships.
...
| Component layout, props | **Code → Figma** (manual) | Designers update Figma components when React components change; reviewer (Elena) verifies parity in PR review |
| New token proposals | **Figma → code** (proposal flow) | Designers propose new tokens by creating a Figma variable in a `_proposed/` collection; Elena reviews; on approval, the token is added to W3C JSON source and the proposed Figma variable is moved into the canonical collection by the next sync |
...
| 27 | Voice and tone match §10.1 / §10.2 (Sophie review) | v3.1 |
...
|--------|---------|-------|--------|
| **Quarterly Adoption Review** | Q1, Q2, Q3, Q4 | Elena | Per-component adoption report; promotion / deprecation candidates list |
| **Twice-yearly Glossary Review** | Q1, Q3 | Sophie | Glossary diff; locale variant updates |
| **Annual Error-message audit** | Q4 | Sophie | All error messages globally audited against §10.5.4 |
| **Annual Token Audit** | Q1 | Priya | Every token's contrastPair, role assignment, and adoption count |
| **Per-release Visual Regression** | Every release | Priya | Visual regression sign-off |
| **Per-release Pattern Recipe Lint** | Every release | Elena | Recipe schema CI report |
...
| Domain | Approver |
|--------|----------|
| Visual / motion / typography | Elena |
| Implementation / API / build / tokens | Priya, Alex |
| Content / voice / glossary | Sophie |
| Privacy / telemetry programme | Sophie + Alex (joint) |
| Cross-product policy | Fiyin |

Single-owner approval is sufficient within a domain; cross-domain changes require both owners. Major-version changes require Fiyin's explicit sign-off.
...
External contributors do not have merge rights; only Priya, Alex, and Elena do. The bar for accepting external code is high — the design system is not a commons, it is a product.
...
|----------|---------------|-------|
| RisqBase Design System | `design.risqbase.com/accessibility` | Elena |
| RALIA | `ralia.io/accessibility` | RALIA team |
| Cortex | `cortex.risqbase.com/accessibility` | Cortex team |
...
A new domain (a sixth) is added only with Fiyin's approval. Five is the working ceiling — more domains fragment imports without aiding navigation.
...
In v4.1 promotion to `core/` required two-property adoption, sound API, accessibility documentation, and Elena's approval. v4.2 makes this **adoption-evidence-based** — promotion requires telemetry-confirmed adoption metrics, not just two checkboxes ticked on a form.
...
|------|------|------|--------|
| Principal Designer | Elena Vasquez | 7 May 2026 | Authored v4.2 |
| Frontend + UI | Priya Sharma | 7 May 2026 | Reviewed |
| Technical Lead | Alex Chen | 7 May 2026 | Reviewed |
| Growth & Customer Success | Sophie Brennan | 7 May 2026 | Reviewed |
| CEO | Fiyin Adeleke | 7 May 2026 | Approved |
