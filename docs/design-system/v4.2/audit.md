...
**F4 honest score:** 3 of 6 fully done on the spec side; the entire F4 implementation surface (hooks, dashboard, warnings, first review meeting) is queued. Telemetry programme cannot ship until Sophie + Alex sign off the DPA disclosure language — that is correctly flagged in §15.7.5.
