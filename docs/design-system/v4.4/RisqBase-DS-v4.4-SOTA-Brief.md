...
| 2026-06-10 | **A11Y-FIX root cause (D-123 reconciliation)** | The five probes **killed C1/C2 as hypothesised**: the dominant dark failure was browser-default black/link-blue text — the Chromatic Storybook **never had a Tailwind pipeline**, so every `text-[var(--…)]` token class and palette utility was unstyled and axe audited unstyled HTML against the theme canvas. Wiring Tailwind (consumer parity: same preset products use) + the documented body contract collapsed the probe set 142 → 14 with zero token derivation changes. Dark token chains were correct all along. | A11Y-FIX D-123; docs/theming.md consumer contract |
| 2026-06-10 | **Valence text tokens** | ClientGrid/MetricCard used band/risk *fills* (3:1 markers) as small TEXT (axe: emerald-500 on white = 2.53). Added `risk.low-text` / `risk.critical-text` (light: emerald-700 / red-700; dark: derived lifts ≥4.5) + new primitive `emerald.700`. | B2 `action.link` precedent (fill vs text contract split) |
| 2026-06-10 | **Choropleth/ImpactGraph `role="group"`** | choropleth-spec §5's `role="img"` makes descendants presentational and trips axe `nested-interactive` once focusable regions/chips live inside; interactive graphics carry `role="group"` instead (non-interactive chart types keep `role="img"`). | D-123: follow confirmed rule IDs over hypothesis |
| 2026-06-10 | **axe `region` rule disabled for stories** | Page-level landmark-containment rule fired on every story canvas (475/668 post-Tailwind nodes) — components are fragments, not pages. Landmark-duplication rules stay on; product-page audits keep `region`. | Component-vs-page scoping (R11 scope-table precedent) |
