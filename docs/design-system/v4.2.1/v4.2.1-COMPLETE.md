...
These items remain post-v4.2.1 and ship under separate work:

| Item | Owner | Lands in |
|---|---|---|
| Hex value selection + AA-contrast verification for 38 TBD primitives in `tokens/primitive/color-chart.json` | Claude Design | BRIEF-401 (PR #29) commissions; values-only PRs follow |
| Dark + HC override values for ~40 reserved-name placeholders enumerated in §15.6.10 | G4 + Claude Design | BRIEF-401; v4.3 if deferred |
| `tokens/themes/print.json` value population | Claude Design | BRIEF-401 |
| `lint:glossary`, `lint:recipes-voice`, `lint:tokens` implementations | G1 + Frontend | Engineering programme per `implementation-plan.md` §5.3 |
...
| `scopes[]` patch across existing semantic + component tokens (per A7) | Claude Design audit + Claude Code | BRIEF-401 Phase 2.5 |

None block product migration; they are the next slice of the engineering programme, commissioned in detail by **BRIEF-401** (`docs/briefs/BRIEF-401-DS-IMPL-AUDIT.md`).
