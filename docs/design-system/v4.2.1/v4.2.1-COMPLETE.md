...
These items remain post-v4.2.1 and ship under separate work. BRIEF-401 (Claude Design audit + value-selection pass, 2026-05-13) closed four of the seven rows; the Chromatic activation (PR #44, 2026-05-14) closed a fifth; remaining two sit on the engineering programme.
...
| ✓ | Chromatic `@media print` baselines | G5 | **DONE in PR #44** (Chromatic activation 2026-05-14). Storybook 8.6 + `chromaui/action@latest`, 19 stories across 5 core components (Button / Badge / Header / Footer / SectionEyebrow), 4-theme matrix (light / dark / HC / print via per-Gallery `chromatic.modes.print = { media: 'print' }`). 62 captures established as the initial baseline; every PR touching `src/` / `tokens/` / `.storybook/` now produces a Chromatic build URL with side-by-side diffs against this baseline. |
