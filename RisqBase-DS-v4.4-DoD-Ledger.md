...
- **chromatic** (required): red **by design** until baselines are accepted — the workflow runs `exitZeroOnChanges: false`, so the job exits 1 whenever changes await review (log: "Found N visual changes … failed with exit code 1"). Current build: **110** (post-D-125; local a11y parity sweep all-zero) — Fiyin's single acceptance click per D-120 turns this check green.
