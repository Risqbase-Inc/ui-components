...
**This ledger IS the done report (DoD-7). Overall status: `BLOCKED`** — every implementable row is PASS (no DESCOPED rows), and the remaining BLOCKED rows sit on five external dependencies: ① Chromatic baseline acceptance (**now actionable** — the D-120 hold is satisfied: post-fix build **111** at `f78ef93` published 200 stories with an all-zero local a11y parity sweep; UI Tests shows 30 changes pending; the acceptance click stays with Fiyin), ② *(resolved 10 Jun — D-125a/b/c applied, sweep all-zero)*, ③ post-merge release pipeline (npm publishes), ④ docs-site deploy (live-URL verification; Vercel project-level rejection), ⑤ Figma Org access (amended D-109, 27 June gate). No row is FAIL.
